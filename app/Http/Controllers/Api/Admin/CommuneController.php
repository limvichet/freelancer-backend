...
class CommuneController extends Controller
{
    public function index(Request $request)
    {
        try {
            ...
            $query   = Commune::orderBy('com_code', 'ASC');
    ...
    public function store(Request $request)
    {
        try {
            $request->validate([
                'com_code'   => 'required|string|unique:sys_location_communes,com_code',
            ...
            return response()->json([
                ...
                'data' =>  Commune::findOrFail($request->com_code),
            ], 201);
    ...
    {
        try {
            $data = Commune::where('com_code', $id)->firstOrFail();
            ...
            $request->validate([
                'dis_code'   => [
                    ...
                    'string',
                    'max:4',
                    Rule::unique('sys_location_communes', 'dis_code')->ignore($data->dis_code, 'dis_code'),
                ],
                 'com_code'   => [
                    'required',
                    'string',
                    'max:6',
                    Rule::unique('sys_location_communes', 'com_code')->ignore($data->com_code, 'com_code'),
                ],
                'name_en'    => 'required|string',
